...
** <em>val</em> denotes validation results, <em>test-dev</em> denotes [test-dev2017](http://cocodataset.org/#upload) server results. AP<sup>test</sup> is for COCO test-dev2017 accuracy, all other AP results in the table are for validation accuracy.  
** All accuracy numbers are for single-model single-scale without ensemble or test-time augmentation.  
** Latency<sub>GPU</sub> measures average end-to-end latency over 5000 COCO test-dev2017 images with a V100 GPU with batch-size 16, image size 640 and confidence threshold 0.1, and includes image preprocessing, inference, postprocessing and NMS. Average NMS time included in this chart is 1.6ms/image.  
